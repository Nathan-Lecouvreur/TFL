...
my_dat_neg$rank <- seq.int(nrow(my_dat_neg))

my_dat_neg$y <- ((my_dat_neg$rank * abs(my_dat_neg$neg)) 
                / (nrow(my_dat_neg)))


my_dat_pos$rank <- seq.int(nrow(my_dat_pos))

my_dat_pos$y <-((my_dat_pos$rank * abs(my_dat_pos$neg)) 
                / (nrow(my_dat_pos)))


if (max(my_dat_pos$y > max(my_dat_neg$y))){
  
  max_neg <- max(my_dat_neg$y)
  
  my_dat_neg$y <- ((max(my_dat_pos$y) 
                    * my_dat_neg$y)
                   / max_neg)
} else {
  
  max_pos <- max(my_dat_pos$y)
  
  my_dat_pos$y <- ((max(my_dat_neg$y) 
                    * my_dat_pos$y)
                   / max_pos)
}
