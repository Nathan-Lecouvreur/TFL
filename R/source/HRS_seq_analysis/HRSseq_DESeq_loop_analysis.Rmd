...
df_counts_global$ratio_global <- df_counts_global_loop$count_global /
...
df_representation_loop$ratio_global <- df_counts_global$ratio_global

df_representation_loop$count_global <- df_counts_global$count_global
```
...
clustering_loop <- df_es_loop_edger_only %>% 
                   group_by(chromosome) %>% 
                   mutate(cluster = Ckmeans.1d.dp(center, c(1, 10 000)))
